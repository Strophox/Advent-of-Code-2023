...
parse txt = listArray (I2 0 0, I2 y x) [read [c] | c<-txt, c/='\n']
  where (y,x) = ((length . lines) txt - 1, (length . head . lines) txt - 1)
...
dijkstra neighbors starts isStop = run M.empty (S.fromList [(0,n) | n<-starts]) where
  run dists pqueue
    ...
    where Just ((dist,node),rest) = S.minView pqueue
          ...
          pqueue' = foldr S.insert rest [(dist+d,n) | (d,n)<-neighbors node, n`M.notMember`dists]
